  ...
  const handleImportCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        // Create a Set of existing entry signatures for O(1) lookup
        // Signature format: "timestamp_type_reps_weight"
        const existingSignatures = new Set(logs.map(log => {
             return `${new Date(log.date).getTime()}-${log.type}-${log.reps}-${log.weight || '0'}`;
        }));

        // Track signatures within the current file to avoid duplicates inside the CSV itself
        ...
        lines.forEach((line, index) => {
          ...
          const weight = (weightStr && !isNaN(parseFloat(weightStr))) ? parseFloat(weightStr) : undefined;

          if (date && !isNaN(date.getTime()) && type && validTypes.has(type) && repsStr && !isNaN(reps) && reps > 0) {
            // Generate signature for this candidate
            const signature = `${date.getTime()}-${type}-${reps}-${weight || '0'}`;
        ...
        if (newLogs.length > 0) {
          ...
          setToastMessage(`✓ Imported ${newLogs.length} logs!${duplicatesSkipped > 0 ? ` (${duplicatesSkipped} skipped)` : ''}`);
          ...
          if (user && supabase && navigator.onLine) {
            ...
            if (duplicatesSkipped > 0) {
                setToastMessage(`No new data. ${duplicatesSkipped} duplicates skipped.`);
            } else {
        ...
        >
          ...
          {activeTab === 'settings' && (
            <div className="space-y-6">
              ...
              <div className="flex flex-col items-center justify-center pb-8 pt-4 gap-4">
                 <div className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">
                    v{APP_VERSION}
                 </div>
                 <a 
                   href="https://github.com/dedied/fittrack-pro" 
                   target="_blank" 
