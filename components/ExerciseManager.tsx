  ...
  return (
    <div className="fixed inset-0 bg-slate-50 z-50 flex flex-col">
      ...
      <div className="p-4 bg-white border-b border-slate-100 space-y-4 flex-shrink-0">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
          <button 
            ...
            className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider whitespace-nowrap transition-all border ${filter === 'ALL' ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-400 border-slate-200'}`}
          >
            All
          </button>
          <button 
            onClick={() => setFilter('ACTIVE')}
            className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider whitespace-nowrap transition-all border ${filter === 'ACTIVE' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-400 border-slate-200'}`}
          >
            Selected ({activeIds.length})
          </button>
          {categories.map(cat => (
             <button 
               key={cat}
               onClick={() => setFilter(cat)}
               className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider whitespace-nowrap transition-all border ${filter === cat ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-400 border-slate-200'}`}
             >
               {cat}
             </button>
          ))}
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="flex gap-2">
                     ...
                     {ex.categories && ex.categories.includes('Gym') && (
                        <span className="text-[9px] bg-slate-100 px-1.5 rounded text-slate-500 font-bold self-center">GYM</span>
                     )}
                  </div>
                </div>
      ...
      <div className="p-4 bg-white border-t border-slate-200 flex-shrink-0 safe-bottom">
        <button onClick={onClose} className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-sm shadow-xl active:scale-[0.98] transition-all">
          Done
        </button>
      </div>
    </div>
